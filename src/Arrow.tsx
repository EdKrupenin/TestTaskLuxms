...
const Arrow: React.FC<ArrowProps> = ({ devRef, testRef, prodRef, devToTestDiff, testToProdDiff }) => {
    ...
    const calculatePosition = useCallback(() => {
        if (devRef.current && testRef.current && prodRef.current) {
            ...
            const devY = DIMENSIONS_EXTENDED.marginTop + devRect.top - parentRect.top;
            ...
            const testY = DIMENSIONS_EXTENDED.marginTop + testRect.top - parentRect.top;
            const prodX = prodRect.left + prodRect.width / 2 - parentRect.left;
            const prodY = DIMENSIONS_EXTENDED.marginTop + prodRect.top - parentRect.top;

            const devToTestPath = `
                M ${devX} ${devY}
                V ${DIMENSIONS_EXTENDED.arrowMinHeight}
                H ${devToTestX}
                V ${testY}
            `;
            ...
            setPathData({
                ...
                devToTestLabelPosition: {
                    x: (devX + devToTestX - DIMENSIONS_EXTENDED.labelWidth) / 2,
                    y: DIMENSIONS_EXTENDED.arrowMinHeight - DIMENSIONS_EXTENDED.labelHeight / 2
                },
                testToProdLabelPosition: {
                    x: (testToProdX + prodX - DIMENSIONS_EXTENDED.labelWidth) / 2,
                    y: DIMENSIONS_EXTENDED.arrowMinHeight - DIMENSIONS_EXTENDED.labelHeight / 2
                }
            });
        }
